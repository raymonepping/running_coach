...
async function reviewLatest(action: "approve" | "reject") {
  const recommendation = dashboard.value?.latestRecommendation;
...
<template>
  ...
  <section class="mt-5 grid gap-5 xl:grid-cols-[1.25fr_0.75fr]">
    <SignalPanel title="Latest autonomous insight">
      ...
      <div v-if="dashboard?.latestRecommendation" class="mt-6 rounded-lg bg-neutral-950 p-5 text-white">
        <div class="mb-2 flex items-center gap-2 text-sm font-medium text-yellow-300">
          <Clock3 :size="16" />
          Next recommended session
        </div>
        <p class="leading-7">{{ dashboard.latestRecommendation.suggested_session }}</p>
        <div class="mt-5 flex flex-wrap gap-3">
          <ActionButton @click="reviewLatest('approve')">
            <template #icon><Check :size="18" /></template>
            Approve
          </ActionButton>
          <ActionButton tone="secondary" @click="reviewLatest('reject')">
    ...
    <SignalPanel title="Proactive import lanes">
      <div class="grid gap-3">
        <NuxtLink class="rounded-lg bg-orange-50 p-4 text-left hover:bg-orange-100" to="/import/activity">
        ...
        <NuxtLink class="rounded-lg bg-yellow-50 p-4 text-left hover:bg-yellow-100" to="/import/sleep">
        ...
        <NuxtLink class="rounded-lg bg-neutral-50 p-4 text-left hover:bg-neutral-100" to="/import/stress">
